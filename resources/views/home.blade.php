...
<section class="bg-brand text-white rounded-2xl px-6 py-12 text-center mb-8">
    <h1 class="text-3xl sm:text-4xl font-bold">{{ __('ui.home.headline') }}</h1>
    <p class="mt-3 text-white/80">{{ __('ui.home.subline') }}</p>

    <form action="{{ route('search') }}" method="GET" class="mt-6 max-w-2xl mx-auto flex">
        <input type="text" name="q" placeholder="{{ __('ui.home.search_placeholder') }}"
               class="w-full rounded-l-lg px-4 py-3 text-gray-800 focus:outline-none">
        <button class="bg-yellow-400 text-brand-dark font-semibold px-6 rounded-r-lg hover:bg-yellow-300">
            {{ __('ui.nav.search') }}
        </button>
    </form>
    ...
    <a href="{{ route('ads.create') }}" class="inline-block mt-5 bg-white text-brand font-semibold px-6 py-2.5 rounded-lg hover:bg-gray-100">
        {{ __('ui.home.post_ad_cta') }}
    </a>
</section>

{{-- Category grid --}}
<section class="mb-10">
    <h2 class="text-xl font-bold mb-4">{{ __('ui.home.browse_categories') }}</h2>
        ...
        @foreach($categories as $cat)
            <a href="{{ route('ads.index', ['category' => $cat->id]) }}"
               class="bg-white rounded-xl shadow hover:shadow-md transition p-4 text-center">
                ...
                <div class="text-xs text-gray-400">{{ $cat->ads_count }} {{ __('ui.home.ads_label') }}</div>
            </a>
        @endforeach
    </div>
</section>
...
<section class="mb-10">
    <h2 class="text-xl font-bold mb-4">{{ __('ui.home.featured_ads') }}</h2>
            ...
            <a href="{{ route('ads.show', $ad) }}" class="bg-white rounded-lg shadow hover:shadow-md overflow-hidden">
                ...
                <div class="p-3">
                    ...
                    <p class="text-brand font-bold text-sm mt-1">
                        @if(!is_null($ad->price)) {{ __('ui.common.rs') }} {{ number_format($ad->price) }} @else - @endif
                    </p>
...
@foreach($sections as $section)
    <section class="mb-10">
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('ads.index', ['category' => $section['category']->id]) }}"
               class="text-brand text-sm hover:underline">{{ __('ui.home.see_all') }}</a>
        </div>
...
@if($sections->isEmpty() && $featured->isEmpty())
    <div class="bg-white rounded-lg shadow p-10 text-center text-gray-500">
        {{ __('ui.home.no_ads') }}
        <a href="{{ route('ads.create') }}" class="text-brand hover:underline ml-1">{{ __('ui.home.post_first_ad') }}</a>
    </div>
@endif
